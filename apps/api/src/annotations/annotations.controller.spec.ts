...
function make(overrides: Partial<AnnotationsService> = {}) {
  const service = {
  ...
  return { service, controller: new AnnotationsController(service) };
}
...
describe('AnnotationsController', () => {
  ...
  it('creates a annotation for the session user', async () => {
    const created = { id: 'n1' } as Annotation;
    const { service, controller } = make({
  ...
  it('updates an owned annotation', async () => {
    const updated = { id: 'n1', text: 'edited' } as Annotation;
    const { service, controller } = make({
  ...
  it('saves a dragged position', async () => {
    const updated = { id: 'n1', offsetX: -120, offsetY: 40 } as Annotation;
    const { service, controller } = make({
      update: vi.fn().mockResolvedValue(updated),
    });
  ...
  it('404s updating a annotation the user does not own', async () => {
    const { controller } = make({
  ...
  it('404s deleting a annotation the user does not own', async () => {
