...
2026-07-14: **#13 PWA + mobile built** — installable PWA (manifest + gold open-book icons incl. maskable, apple-touch-icon, theme-color; SW precaches NOTHING — install-only, no offline per PRD; sw.js copied into build/client by the build script), and the mobile pass: viewports <640px scale the print-fixed sheet to fit (transform + height compensation — never reflows), all mark layers/drag/page-jump measure scale-aware, sheet re-centers when scale lands, jump box hidden on phones. Verified with headless-Chromium screenshots (phone 390x844 + desktop 1440x900): sheet scaled+centered, highlight wash + ink circle land on the right words at scale, desktop untouched. Still needs a REAL phone check (touch selection -> menu -> highlight/annotate) before closing #13.
